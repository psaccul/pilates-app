...
const emptyForm = { nombre:'', apellido:'', telefono:'', plan:'mensual', frecuencia:'', instructor_id:'', notas:'' }
...
export default function Alumnos() {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('open-alumno', h1)
      window.removeEventListener('alumno-actualizado', h2)
    }
  }, [])
  ...
  function abrirFicha(alumnoId) {
    window.dispatchEvent(new CustomEvent('open-ficha-alumno', { detail: alumnoId }))
  }
  ...
  async function abrirHorarios(alumno) {
    ...
    const { data } = await supabase.from('horarios_alumno')
      ...
      .eq('alumno_id', alumno.id).eq('activo', true)
  ...
  async function replicarClases() {
    ...
    setReplicando(true)
    setReplicarOk(false)
    const { addDays, addWeeks, startOfWeek } = await import('date-fns')
    ...
    for (let semana = 0; semana < replicarSemanas; semana++) {
      for (const h of horarios) {
        ...
        const { data: existe } = await supabase.from('clases')
          .select('id').eq('fecha', fechaStr).eq('hora', h.hora)
          .eq('instructor_id', h.instructor_id || '').maybeSingle()

        let claseId
        if (!existe) {
          const { data: nueva } = await supabase.from('clases').insert({
            nombre: h.nombre_clase, tipo: 'grupal',
            instructor_id: h.instructor_id || null,
            fecha: fechaStr, hora: h.hora, sala: h.sala, capacidad: 4,
          }).select().single()
          claseId = nueva?.id
        } else {
          claseId = existe.id
        ...
        if (claseId) {
          await supabase.from('asistencias').upsert({
            clase_id: claseId, alumno_id: horariosModal.id,
    ...
    <>
      ...
      <div className="panel">
        ...
        {loading ? <div className="loading">Cargando…</div> : (
          <table className="tbl">
            <thead>
              <tr><th>Alumno</th><th>Plan</th><th>Instructor</th><th>Teléfono</th><th>Notas</th><th>Estado</th><th></th></tr>
            </thead>
            <tbody>
                  ...
                  <tr key={a.id}>
                    <td>
                      <div style={{display:'flex', alignItems:'center', gap:8, cursor:'pointer'}}
                        ...
                        <span style={{fontWeight:500, color:'var(--mg)'}}>{a.nombre} {a.apellido}</span>
                      </div>
                    </td>
                    <td>{a.plan==='mensual'?'Mensual':'Sueltas'}{a.frecuencia?` · ${a.frecuencia}`:''}</td>
                    <td>{a.instructores?`${a.instructores.nombre} ${a.instructores.apellido}`:'—'}</td>
                    <td>{a.telefono||'—'}</td>
                    <td>
                      {a.notas
                        ? <span title={a.notas} style={{fontSize:11, color:'var(--sl-m)', cursor:'help'}}>
                            {a.notas.length > 30 ? a.notas.slice(0,30)+'…' : a.notas}
                          </span>
                    ...
                    <td style={{display:'flex', gap:5, flexWrap:'wrap'}}>
                      <button className="btn-sec" style={{fontSize:11,padding:'4px 8px'}} onClick={() => abrirHorarios(a)}>Horarios</button>
                      <button className="btn-sec" style={{fontSize:11,padding:'4px 8px'}} onClick={() => verHistorial(a)}>Historial</button>
                      <button className="btn-sec" style={{fontSize:11,padding:'4px 8px'}} onClick={() => openModal(a)}>Editar</button>
                      <button className="btn-danger" style={{fontSize:11,padding:'4px 8px'}} onClick={() => handleDelete(a.id)}>✕</button>
                    </td>
                  </tr>
                )
              })}
      ...
      {modal && (
        <Modal title={form.id?'Editar alumno':'Nuevo alumno'}
          ...
          footer={<>
            ...
            <button className="btn-pri" onClick={handleSave} disabled={saving}>{saving?'Guardando…':'Guardar alumno'}</button>
          </>}>
          ...
          <div className="form-row">
            ...
            <textarea className="form-inp" value={form.notas} onChange={set('notas')}
              placeholder="Ej: Hernia lumbar L4-L5. Evitar flexión profunda. Embarazada 20 semanas..." />
          </div>
        </Modal>
      )}

      {/* MODAL: Horarios fijos */}
      {horariosModal && (
        <Modal title={`Horarios — ${horariosModal.nombre} ${horariosModal.apellido}`}
          ...
          {horarios.length === 0
            ? <div className="empty" style={{padding:'16px 0'}}>Sin horarios fijos cargados</div>
            : horarios.map(h => (
              <div key={h.id} style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 0', borderBottom:'1px solid var(--border)'}}>
                <div>
                  ...
                  <div style={{fontSize:11, color:'var(--sl-m)'}}>{h.nombre_clase} · {h.sala} · {h.instructores?`${h.instructores.nombre} ${h.instructores.apellido}`:'—'}</div>
                </div>
                <button className="btn-danger" style={{fontSize:11,padding:'3px 8px'}} onClick={() => eliminarHorario(h.id)}>✕</button>
              </div>
            ))
          }
          ...
          <div style={{marginTop:16, padding:'14px', background:'var(--sl-l)', borderRadius:10}}>
            ...
            <button className="btn-pri" style={{marginTop:10, fontSize:12}} onClick={agregarHorario} disabled={saving}>
              {saving?'Agregando…':'+ Agregar horario'}
            </button>
          </div>
            ...
            <div style={{marginTop:16, padding:'14px', background:'#FEF3E2', borderRadius:10, border:'1px solid #F0C060'}}>
              ...
              <div className="form-row2">
                <div className="form-row" style={{marginBottom:0}}>
                  ...
                  <input className="form-inp" type="date" value={replicarDesde} onChange={e => setReplicarDesde(e.target.value)} />
                </div>
                <div className="form-row" style={{marginBottom:0}}>
                  <label className="form-lbl">Semanas</label>
                  <select className="form-inp" value={replicarSemanas} onChange={e => setReplicarSemanas(Number(e.target.value))}>
              ...
              {replicarOk && (
                <div style={{fontSize:12, color:'#2D7A5A', background:'#E4F4EE', padding:'8px 12px', borderRadius:7, marginBottom:10}}>
                  ¡Clases generadas! Ya aparecen en el calendario.
                </div>
              )}
              <button className="btn-pri" style={{fontSize:12, background:'#D4A020', marginTop:8}}
                onClick={replicarClases} disabled={replicando}>
                {replicando?'Generando…':`Generar ${replicarSemanas} semanas`}
              </button>
            </div>
          )}
        </Modal>
