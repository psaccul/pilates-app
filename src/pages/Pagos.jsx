...
export default function Pagos() {
  ...
  async function handleSave() {
    ...
    if (form.id) {
      await supabase.from('pagos').update(payload).eq('id', form.id)
    } else {
  ...
  async function togglePagado(pago) {
    ...
    await supabase.from('pagos').update({
      pagado: nuevo,
      fecha_pago: nuevo ? format(new Date(),'yyyy-MM-dd') : null
    }).eq('id', pago.id)
    setPagos(prev => prev.map(p => p.id === pago.id
      ? { ...p, pagado: nuevo, fecha_pago: nuevo ? format(new Date(),'yyyy-MM-dd') : null }
      : p
    ))
  }
  ...
  const totalPagado   = pagos.filter(p => p.pagado).reduce((s,p) => s + (p.monto||0), 0)
  const totalPendiente = pagos.filter(p => !p.pagado).reduce((s,p) => s + (p.monto||0), 0)
    ...
    <>
      <div className="stats" style={{gridTemplateColumns:'repeat(3,1fr)'}}>
        <div className="sc" style={{'--acc':'var(--teal)'}}>
          ...
          <div className="sc-val" style={{fontSize:24}}>
      ...
      <div className="tabs">
        {['todos','pagados','pendientes'].map(f => (
          <div key={f} className={`tab${filtro===f?' active':''}`}
            onClick={() => setFiltro(f)}>
      ...
      <div className="panel">
        ...
        {loading ? <div className="loading">Cargando…</div> : (
          <table className="tbl">
            <thead>
              <tr>
                <th>Alumno</th><th>Concepto</th><th>Monto</th>
                <th>Medio</th><th>Fecha pago</th><th>Estado</th><th></th>
              </tr>
            </thead>
            <tbody>
              {filtrados.length === 0 && (
                ...
                <tr key={p.id}>
                  <td>
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      <Avatar
                        nombre={p.alumnos?.nombre||'?'}
                        apellido={p.alumnos?.apellido||''}
                        size={24} fontSize={8}
                      />
                      <span>{p.alumnos ? `${p.alumnos.nombre} ${p.alumnos.apellido}` : '—'}</span>
                    </div>
                  </td>
                  <td>{p.concepto}</td>
                  <td style={{fontWeight:500}}>
                    {p.monto != null ? `$${Number(p.monto).toLocaleString('es-AR')}` : '—'}
                  </td>
                  <td>{medioTag(p.medio)}</td>
                  <td style={{fontSize:11,color:'var(--sl-m)'}}>
                    {p.fecha_pago
                      ? format(new Date(p.fecha_pago+'T00:00:00'),'dd/MM/yy')
                      : '—'}
                  </td>
                  <td>
                    <Toggle
                      value={p.pagado}
                      onChange={() => togglePagado(p)}
                      labelOn="Pagado"
                      labelOff="Pendiente"
                    />
                  </td>
      ...
      {modal && (
        ...
        >
          ...
          <div className="form-row">
            ...
            <input className="form-inp" value={form.concepto} onChange={set('concepto')}
              placeholder="Ej: Plan mensual — Mayo" />
          </div>
          <div className="form-row2">
            <div className="form-row" style={{marginBottom:0}}>
              <label className="form-lbl">Monto</label>
              <input className="form-inp" type="number" value={form.monto} onChange={set('monto')}
                placeholder="0" />
            </div>
            <div className="form-row" style={{marginBottom:0}}>
          ...
          <div className="form-row">
            ...
            <div style={{marginTop:6}}>
              <Toggle
                value={form.pagado}
                onChange={v => setForm(f => ({ ...f, pagado: v }))}
                labelOn="Sí, ya pagó"
                labelOff="Pendiente"
              />
            </div>
