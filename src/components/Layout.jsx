...
const PAGE_TITLES = {
  ...
  instructores:'Instructores', pagos:'Pagos', reportes:'Reportes', finanzas:'Finanzas · Gerencia'
}
...
export default function Layout() {
  ...
  const pages = esGerente
    ? [...PAGES_BASE, { id:'finanzas', label:'Finanzas ★', icon:'◆', soloGerente:true }]
    : PAGES_BASE
  ...
  async function fetchNotifs() {
    ...
    const { data: packs } = await supabase.from('packs')
      .select('*, alumnos(nombre,apellido)').eq('activo',true)
    ;(packs||[]).forEach(pk => {
      ...
      if (restantes <= (pk.alerta_clases_restantes||2)) {
        nuevas.push({ tipo:'pack', icon:'📦', color:'#B03030', bg:'#FDECEA',
          ...
          desc:`${pk.alumnos?.nombre} ${pk.alumnos?.apellido} — ${restantes} clase${restantes!==1?'s':''} restante${restantes!==1?'s':''}`,
    ...
    // Ausentes sin aviso hoy
    const { data: sinAviso } = await supabase.from('asistencias')
      .select('id, alumnos:alumno_id(nombre,apellido), clases(nombre,fecha)')
      .eq('estado_asistencia','ausente_sin_aviso')
  ...
  return (
    <div className="layout">
      ...
      <aside className={`sidebar${sidebarOpen?' open':''}`}>
        ...
        <nav>
          {pages.map(p => (
            <div key={p.id} className={`nav-item${page===p.id?' active':''}${p.soloGerente?' nav-gerente':''}`}
              onClick={() => navTo(p.id)}>
        ...
        <div className="topbar">
          <div style={{display:'flex',alignItems:'center',gap:10,minWidth:0}}>
            ...
            <div style={{minWidth:0}}>
              <div className="tb-title" style={{whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{PAGE_TITLES[page]}</div>
              ...
              {notifOpen && (
                <div className="notif-dropdown">
                  <div className="notif-drop-header">
                    ...
                    <span style={{fontSize:11,color:'var(--sl-m)'}}>{notifs.length} nueva{notifs.length!==1?'s':''}</span>
                  </div>
                  {notifs.length===0 && <div className="empty" style={{padding:'20px'}}>Sin alertas nuevas</div>}
                  {notifs.map((n,i) => (
                    <div key={i} className="notif-item" style={{cursor:n.alumnoId?'pointer':'default'}}
                      ...
                      <div className="notif-ic" style={{background:n.bg,fontSize:14}}>{n.icon}</div>
          ...
          {!fichaEdit ? (
            <>
              <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:18}}>
                <Avatar nombre={fichaData.nombre} apellido={fichaData.apellido} size={46} fontSize={15} />
                <div>
                  <div style={{fontSize:17,fontWeight:500}}>{fichaData.nombre} {fichaData.apellido}</div>
              ...
              <table style={{width:'100%',fontSize:13,borderCollapse:'collapse'}}>
                {[
                  ...
                  <tr key={k} style={{borderBottom:'1px solid var(--border)'}}>
                    <td style={{padding:'7px 0',color:'var(--sl-m)',width:'38%'}}>{k}</td>
              ...
              {(fichaData.packs||[]).filter(p=>p.activo).map(pk => {
                ...
                return (
                  <div key={pk.id} style={{marginTop:10,padding:'10px 12px',background:rest<=2?'#FDECEA':'#E4F4EE',borderRadius:8}}>
                    <div style={{display:'flex',justifyContent:'space-between',fontSize:12,fontWeight:500}}>
                      <span>Pack: {pk.nombre}</span>
                      <span style={{color:rest<=2?'#B03030':'#2D7A5A',fontFamily:'var(--font-num)'}}>{rest} clase{rest!==1?'s':''} restante{rest!==1?'s':''}</span>
                    </div>
              ...
              {fichaData.notas && (
                <div style={{marginTop:12,padding:'10px 12px',background:'var(--sl-l)',borderRadius:8}}>
                  <div style={{fontSize:10,color:'var(--sl-m)',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:5}}>Notas / Patologías</div>
            ...
            <>
              ...
              <div className="form-row">
                <label className="form-lbl">Notas / Patologías</label>
...
function PageAction({ page, esGerente }) {
  const actions = {
    dashboard:    { label:'+ Turno',      event:'open-turno' },
    alumnos:      { label:'+ Alumno',     event:'open-alumno' },
    instructores: { label:'+ Instructor', event:'open-instructor' },
    pagos:        { label:'+ Pago',       event:'open-pago' },
  }
